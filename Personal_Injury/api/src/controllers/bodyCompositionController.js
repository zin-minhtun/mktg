...
exports.addBodyComposition = async (req, res) => {
  const { userId, weight, unit, height, neck, waist, hips, bodyFatPercentage, date, notes } = req.body;
...
exports.getBodyCompositions = async (req, res) => {
  try {
    ...
    const bodyCompositions = await BodyComposition.find({ userId }).sort({ date: -1 });
