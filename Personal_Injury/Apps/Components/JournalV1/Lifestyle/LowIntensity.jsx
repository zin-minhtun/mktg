				...
				<View className="flex-row justify-end mt-3">
					...
					<View
						className={`justify-center items-center mr-3 rounded-full w-10 h-10 ${
							LowIntensityNote.length > 0 ? "bg-[#FDA29B80]" : "bg-[#E0E0E0]"
						}`}
					>
					...
					<TouchableOpacity
						onPress={() => null}
						disabled={LowIntensityNote.length === 0}
						className={`justify-center items-center rounded-full w-10 h-10 ${
							LowIntensityNote.length > 0 ? "bg-primaryBlue" : "bg-[#E0E0E0]"
						}`}
					>
						<CheckIcon isActive={LowIntensityNote.length > 0} />
					</TouchableOpacity>
				</View>
			</View>
