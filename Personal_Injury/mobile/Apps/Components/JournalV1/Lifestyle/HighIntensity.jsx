...
const HighIntensity = ({ prefill, editMode, selectedDate }) => {
    const { theme } = useTheme();
    const { gUser } = useUserData();
    const navigation = useNavigation();
    const [selectedOptions, setSelectedOptions] = useState([]);
    const [HighIntensityNote, setHighIntensityNote] = useState("");
    ...
    useEffect(() => {
        if (prefill) {
            setHighIntensityNote(prefill.notes || "");
    ...
    const handleCancel = () => {
        setHighIntensityNote("");
        Keyboard.dismiss();
    };

    const handleAccept = async () => {
        if (!HighIntensityNote && selectedOptions.length === 0) {
            ...
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} className="flex-1 px-5 pt-8 pb-28">
                ...
                <View className="mt-8">
                    ...
                    <StyledTextInput
                        multiline
                        ref={noteRef}
                        value={HighIntensityNote}
                        onChangeText={setHighIntensityNote}
                    ...
                    <View className="flex-row justify-end mt-3">
                        ...
                        <TouchableOpacity
                            ...
                            style={{ backgroundColor: HighIntensityNote.length > 0 ? "#FDA29B80" : (theme.mode === 'dark' ? '#003361' : '#E0E0E0') }}
                        >
                            <CrossIcon isActive={HighIntensityNote.length > 0} />
                        </TouchableOpacity>
                        <TouchableOpacity
                            onPress={handleAccept}
                            disabled={loading}
                            className={`justify-center items-center rounded-full w-10 h-10`}
                            style={{ backgroundColor: (HighIntensityNote.length > 0 || selectedOptions.length > 0) && !loading ? "#22C55E" : (theme.mode === 'dark' ? '#003361' : '#E0E0E0') }}
                        >
                            {loading ? <ActivityIndicator size="small" color="#FFF" /> : <CheckIcon isActive={HighIntensityNote.length > 0 || selectedOptions.length > 0} />}
                        </TouchableOpacity>
                    </View>
                </View>
