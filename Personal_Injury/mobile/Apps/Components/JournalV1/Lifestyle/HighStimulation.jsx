...
const HighStimulation = ({ prefill, editMode, selectedDate }) => {
    const { theme } = useTheme();
    const { gUser } = useUserData();
    const navigation = useNavigation();
    const [selectedOptions, setSelectedOptions] = useState([]);
    const [highStimulationNote, setHighStimulationNote] = useState("");
    ...
    useEffect(() => {
        if (prefill) {
            setHighStimulationNote(prefill.notes || "");
    ...
    const handleCancel = () => { setHighStimulationNote(""); Keyboard.dismiss(); };

    const handleAccept = async () => {
        if (!highStimulationNote && selectedOptions.length === 0) {
            ...
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} className="flex-1 px-5 pt-8 pb-28">
                ...
                <View className="mt-8">
                    ...
                    <StyledTextInput
                        ...
                        onChangeText={setHighStimulationNote}
                    ...
                    <View className="flex-row justify-end mt-3">
                        ...
                        <TouchableOpacity
                            ...
                            style={{ backgroundColor: highStimulationNote.length > 0 ? "#FDA29B80" : (theme.mode === 'dark' ? '#003361' : '#E0E0E0') }}
                        >
                        ...
                        <TouchableOpacity
                            ...
                            style={{ backgroundColor: (highStimulationNote.length > 0 || selectedOptions.length > 0) && !loading ? "#22C55E" : (theme.mode === 'dark' ? '#003361' : '#E0E0E0') }}
                        >
                            {loading ? <ActivityIndicator size="small" color="#FFF" /> : <CheckIcon isActive={highStimulationNote.length > 0 || selectedOptions.length > 0} />}
                        </TouchableOpacity>
                    </View>
                </View>
